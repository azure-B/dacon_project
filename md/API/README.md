...
- Base URL: `/api` (로컬 Express `http://localhost:5000`, Vite 프록시 `http://localhost:3000/api`)
...
Supabase Auth·DB 스키마·환경변수는 **`md/API/supabase.md`**, SQL은 **`back/sql/001_init.sql`** 을 참고합니다.  
...
| 429 | 로그인 실패 횟수 초과 |
| 503 | Supabase 미설정·service_role 키 오류 |
...
| 가계부 목록 | `GET` | `/api/account-book` | Y |
| 시뮬레이션 | `POST` | `/api/simulation` | Y |
...
| --- | --- | --- |
| `demo01` | `pass1234` | `demo01@example.com` |

---

## 3. 시뮬레이션

`POST /api/simulation` (인증 필요)

### Request

```json
{
  "prompt": "보너스 500만원을 빚 갚는데 쓰면?",
  "scenario": "custom",
  "range": "1y"
}
```

- `scenario`: `hold` | `save` | `repay` | `rate` | `custom` (직접 질문. 기본값 `custom`)
- `range`: `3m` | `6m` | `1y`
...
{
  "insight": "...",
  "assetGapLabel": "빚 −500만 원",
  "goalAcceleration": "빚이 더 빨리 줄어요",
  "annualReturn": "연 5% 정도로 가정",
  "strategy": "...",
  "scenarioLabel": "보너스 500만원을 빚 갚는데 쓰면?",
  "rangeLabel": "1년",
  "chart": {
    "assetPath": "M0,...",
    "holdPath": "M0,...",
...
- 관련 파일: `back/routes/simulation.routes.js`, `back/controller/simulation.controller.js`, `back/models/simulation.model.js`
...
- 시뮬레이션: `back/routes/simulation.routes.js`
